...
class Order(models.Model):
    ...
    state = models.CharField(choices=(('completed', u'已完成'), ('pending', u'待处理'), ('closed', u'已关闭')), max_length=10, verbose_name=u'订单状态', default='pending')
    ...
    def actual_slaes_section(self):
        return Decimal.to_integral(self.rebate_amount()+self.compulsory_rebate_amount())
    ...
    def outstanding_corporation(self):
        return Decimal.to_integral(self.insurance_amount / Decimal.from_float(1.06)*self.actual_rebate*Decimal.from_float(1-0.085)
                + self.compulsory_insurance_amount / Decimal.from_float(1.06)*self.compulsory_actual_rebate*Decimal.from_float(1-0.085))
